...
export default function AdminRenterEkycScreen() {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await api.get('/auth/renter-applications');
      setApplications(response.data.applications || response.data || []);
    } catch (error) {
      console.log('Fetch renter ekyc error:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerify = async (id, status) => {
    try {
      await api.put(`/auth/renter-applications/${id}/verify`, { status });
      Alert.alert('Thành công', `Đã ${status === 'approved' ? 'duyệt' : 'từ chối'} hồ sơ Renter`);
      fetchApplications();
    } catch (error) {
  ...
  return (
    <View className="flex-1 bg-gray-50 p-4">
      <Text className="text-2xl font-bold text-gray-800 mb-4">Duyệt eKYC Renter</Text>
